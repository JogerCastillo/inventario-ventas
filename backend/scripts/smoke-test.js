...
function waitForServer(childProcess) {
  return new Promise((resolve, reject) => {
    const onData = (data) => {
      ...
      if (text.includes('API activa en')) {
        cleanup();
        resolve();
      }
    };
...
async function main() {
  if (!process.env.JWT_SECRET || !process.env.ADMIN_PASSWORD) {
    throw new Error('Environment required: JWT_SECRET and ADMIN_PASSWORD must be set for the smoke test.');
  }
  ...
  try {
    ...
    assert.equal(health.body.service, 'inventario-ventas-api');

    const login = await request('/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'admin@inventario.local', password: process.env.ADMIN_PASSWORD })
    });
    ...
    assert.equal(me.body.user.email, 'admin@inventario.local');
